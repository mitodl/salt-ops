...
{% set cassandra_admin_password = salt.random.get_str(42) %}
...
generate_cloud_map_file:
  file.managed:
    - name: /etc/salt/cloud.maps.d/{{ VPC_RESOURCE_SUFFIX }}_cassandra_map.yml
...
deploy_cassandra_nodes:
  salt.runner:
    - name: cloud.map_run
    - path: /etc/salt/cloud.maps.d/{{ VPC_RESOURCE_SUFFIX }}_cassandra_map.yml
    - kwargs:
        parallel: True
    - require:
