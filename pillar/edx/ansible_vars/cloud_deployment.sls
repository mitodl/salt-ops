...
{% set rds_endpoint = salt.boto_rds.get_endpoint('{env}-rds-mysql'.format(env=environment)) %}
{% set MYSQL_HOST = rds_endpoint.split(':')[0] %}
{% endif %}
{% set MYSQL_PORT = 3306 %}
...
edx:
  ...
  ansible_vars:
    ### EDXAPP ENVIRONMENT ###
    EDXAPP_LOGIN_REDIRECT_WHITELIST: {{ purpose_data.domains.values()|list|tojson }}
    EDXAPP_MEMCACHE:
