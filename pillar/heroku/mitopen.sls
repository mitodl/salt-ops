...
{% set env_dict = {
    ...
    'production': {
      ...
      'OPENSEARCH_URL': 'https://search-opensearch-mitopen-productio-34bmfnsi2zdrngie5vdpe7keu4.us-east-1.es.amazonaws.com',
      'PGBOUNCER_DEFAULT_POOL_SIZE': 50,
      'PGBOUNCER_MAX_CLIENT_CONN': 500,
      'PGBOUNCER_MIN_POOL_SIZE': 20,
...
heroku:
  ...
  config_vars:
    ...
    DATABASE_URL: postgres://{{ pg_creds.data.username }}:{{ pg_creds.data.password }}@{{ 'ol-mitopen-db-{}.cbnm7ajau6mi.us-east-1.rds.amazonaws.com'.format(env_data.env_stage) }}:5432/mitopen
